...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Square(uint16 x0, uint16 y0, uint16  with, TCOLOR  color)
{   if(with==0) return;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Line(uint16 x0, uint16 y0, uint16 x1, uint16 y1, TCOLOR color)
{
 GUI_LineWith(x0,y0,x1,y1,1,color);
}
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_LineWith(uint16 x0, uint16 y0, uint16 x1, uint16 y1, uint8 with, TCOLOR color)
{  
	int32   dx;						// 直线x轴差值变量
	int32   dy;          			// 直线y轴差值变量
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_LineS(uint16 const *points, uint8 no, TCOLOR color)
{
	uint16  x0, y0;
	uint16  x1, y1;
	uint8  i;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Circle(uint16 cx, uint16 cy, uint16 r, TCOLOR color)
{
	u16 x,y;
	u8 fill = 0;
	s16 delta,tmp;
	...
	while(y>x)
	{
		...
		{
			GUI_Point(cx+x,cy+y,color);
			GUI_Point(cx-x,cy+y,color);
			GUI_Point(cx+x,cy-y,color);
			GUI_Point(cx-x,cy-y,color);
			GUI_Point(cx+y,cy+x,color);
			GUI_Point(cx-y,cy+x,color);
			GUI_Point(cx+y,cy-x,color);
			GUI_Point(cx-y,cy-x,color);
		}
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_CircleFill(uint16 cx, uint16 cy, uint16 r, TCOLOR color)
{
	u16 x,y;
	u8 fill = 1;
	s16 delta,tmp;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Ellipse(uint16 x0, uint16 x1, uint16 y0, uint16 y1, TCOLOR color)
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_EllipseFill(uint16 x0, uint16 x1, uint16 y0, uint16 y1, TCOLOR color)
{  int32  draw_x0, draw_y0;			// 刽图点坐标变量
   int32  draw_x1, draw_y1;
   int32  draw_x2, draw_y2;
   int32  draw_x3, draw_y3;
...
* 说明：若没有找出，则返回最左的x坐标0。
****************************************************************************/
uint16  GUI_ReadLeftPoint(uint16 x0, uint16 y0, TCOLOR color)
{  uint16  i;
   TCOLOR  bakc;
...
****************************************************************************/
uint16  GUI_ReadRightPoint(uint16 x0, uint16 y0, TCOLOR color)
{  uint16  i;
   TCOLOR  bakc;
...
* 说明：
****************************************************************************/
int  GUI_CmpPointColor(uint16 x, uint16 y, TCOLOR color)
{  TCOLOR  bakc;
...
****************************************************************************/
void  GUI_FloodFill(uint16 x0, uint16 y0, TCOLOR color)
{  PointXY  down_point[DOWNP_N];	// 定义向下填充转折点缓冲区
   ...
   uint16  xx, yy;					// 填充临时x，y变量 (当前填充行的中点)
   uint16  xx0;						// 当前填充行的左x值变量
   uint16  xx1;						// 当前填充行的右y值变量
   uint16  i;
   
   uint16  x0_bak, y0_bak;
   uint16  x1_bak;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Arc4(uint16 x, uint16 y, uint16 r, uint8 angle, TCOLOR color)
{  int32  draw_x, draw_y;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Arc(uint16 x, uint16 y, uint16 r, uint16 stangle, uint16 endangle, TCOLOR color)
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Pieslice(uint16 x, uint16 y, uint16 r, uint16 stangle, uint16 endangle, TCOLOR color)
{  int32  draw_x, draw_y;					// 画图坐标变量
   int32  op_x, op_y;						// 操作坐标
   int32  op_2rr;							// 2*r*r值变量
